...
import edu.aku.omarshoaib.renew.adapter.SectionF06Adapter;
import edu.aku.omarshoaib.renew.database.AppDatabase;
import edu.aku.omarshoaib.renew.databinding.ActivitySection6ListBinding;
import edu.aku.omarshoaib.renew.global.AppConstants;
import edu.aku.omarshoaib.renew.global.AppTextWatcher;
import edu.aku.omarshoaib.renew.global.MainApp;

public class SectionF06ListAC extends BaseActivity {

    private final String TAG = getClass().getSimpleName();
    private final Activity activity = SectionF06ListAC.this;

    ActivitySection6ListBinding bi;
    private AppDatabase appDatabase;
//    private Loading loading;

    private SectionF06Adapter followup2bAdapter;

    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        bi = DataBindingUtil.setContentView(activity, R.layout.activity_section_6_list);
...
//        loading = new Loading(activity, false);
    ...
    private void initUI() {
        MainApp.vFormF06List = appDatabase.vFormF06Dao().getAllData();
...
//        loading.showLoading();
        ...
        new Handler().post(() -> {
            if (MainApp.vFormF06List != null) {
                followup2bAdapter = new SectionF06Adapter(activity, MainApp.vFormF06List);
