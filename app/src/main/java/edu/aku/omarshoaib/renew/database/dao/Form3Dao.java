...
@Dao
public abstract class Form3Dao implements BaseDao<Form3> {

    @Query("SELECT * FROM Form3")
    public abstract List<Form3> getAllData();

    // This query is used to check if the data in the table exists
    @Query("SELECT * FROM Form3 Order by id DESC Limit 1")
    public abstract Form3 isDataExists();

    @Query("SELECT * FROM Form3 WHERE uid IN (:uIds)")
    public abstract List<Form3> getAllUnSyncedDataByUIds(List<String> uIds);
    ...
    @Query("UPDATE Form3 SET iStatus = :iStatus, iStatus96x = :iStatus96x, isFormCompleteOnce = :isFormCompleteOnce, endingDate = :endingDate WHERE id = :id")
    public abstract void updateIStatus(long id, String iStatus, String iStatus96x, boolean isFormCompleteOnce, String endingDate);

    @Query("SELECT EXISTS(SELECT 1 FROM Form3 WHERE districtCode = :districtCode AND scrId = :checkId AND synced = '1')")
    ...
    public abstract Form3 getDataById(long id);
    ...
    @Query("SELECT COUNT(*) FROM Form3 WHERE username = :username")
    public abstract int getTotalCountByUsername(String username);

    // Get total count
    @Query("SELECT COUNT(*) FROM Form3")
    public abstract int getTotalCount();

    @Query("DELETE FROM Form3")
    ...
    public void updateSyncSuccess(List<SyncModel.WebResponse> responses) {
        if (responses != null && !responses.isEmpty() && responses.get(0).getError() == 0) {
            ...
            for (int i = 0; i < responses.size(); i++) {
                Form3 forms = getDataById(responses.get(i).getId());
    ...
    public void updateSyncError(List<Form3> list) {
        for (int i = 0; i < list.size(); i++) {
            Form3 obj = list.get(i);
            obj.setError(true);
            update(obj);
        }
    }
